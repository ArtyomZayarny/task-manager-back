...
let AuthService = exports.AuthService = class AuthService {
    ...
    async validateUser(email, password) {
        ...
        if (!user) {
            throw new common_1.UnauthorizedException(auth_constants_1.USER_NOT_FOUNDED);
        }
        const isPasswordMatch = await (0, bcrypt_1.compare)(password, user.passwordHash);
        if (!isPasswordMatch) {
            throw new common_1.UnauthorizedException(auth_constants_1.WRONG_PASSWORD);
        }
    ...
    async createUser(dto) {
        ...
        return await this.login((await newSavedUser).email, (await newSavedUser)._id);
    }
};
